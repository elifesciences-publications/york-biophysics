function [SpotsCh1, SpotsCh2, frame_average] = FinalCode12preloaded(image)

% Final code for opening tif data, calculating frame average, 
%, then looping over user defined frames, finding spots,
%identifying their centres and accepting them if they meet criteria, then
%linking these spots together into trajectories.


%Parameters for running preloaded
ImageSize=size(image);
numFrames=ImageSize(1,3);
frame_Ysize=ImageSize(1,1);
frame_Xsize=ImageSize(1,2);
...
exclude_region = 0; % HM must be set to zero if using only one channel

%Channels to use
start_channel=1;
end_channel=1; %HM set start and end channel to 1 if only 1 channel
...
%                         spot_num=spot_num+1;
         ...
         %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         spots_temp(spots_temp(:,1)==0,:)=[]; %HM comment this gets rid of empty rows
 %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
